...
const Navbar = () => {
  ...
  const { mode, toggleMode } = usePerformance();
    ...
    <>
      ...
      >
          ...
          >
            <div className="flex items-center justify-between">
              <div className="hidden md:flex space-x-4">
                <Link
                  to="/"
                  className={`relative block transition hover:text-teal-400 px-1.5 py-2 nav-link ${activeLink === "home" ? "text-teal-400" : ""
                    }`}
                  onClick={(e) => handleLinkClick(e, "home")}
                >
                  Home
                  <span
                    className={`absolute inset-x-0 -bottom-[0.2rem] h-px bg-gradient-to-r from-teal-400/0 via-teal-400/40 to-teal-400/0 transition-opacity duration-300 ease-in-out ${activeLink === "home" ? "opacity-100" : "opacity-0"
                      }`}
                  ></span>
                </Link>
                <Link
                  to="/"
                  className={`relative block transition hover:text-teal-400 px-1.5 py-2 nav-link ${activeLink === "offer" ? "text-teal-400" : ""
                    }`}
                  onClick={(e) => handleLinkClick(e, "offer")}
                >
                  Offer
                  <span
                    className={`absolute inset-x-0 -bottom-[0.2rem] h-px bg-gradient-to-r from-teal-400/0 via-teal-400/40 to-teal-400/0 transition-opacity duration-300 ease-in-out ${activeLink === "offer" ? "opacity-100" : "opacity-0"
                      }`}
                  ></span>
                </Link>
                <Link
                  to="/"
                  className={`relative block transition hover:text-teal-400 px-1.5 py-2 nav-link ${activeLink === "experience" ? "text-teal-400" : ""
                    }`}
                  onClick={(e) => handleLinkClick(e, "experience")}
                >
                  Experience
                  <span
                    className={`absolute inset-x-0 -bottom-[0.2rem] h-px bg-gradient-to-r from-teal-400/0 via-teal-400/40 to-teal-400/0 transition-opacity duration-300 ease-in-out ${activeLink === "experience" ? "opacity-100" : "opacity-0"
                      }`}
                  ></span>
                </Link>
                <Link
                  to="/"
                  className={`relative block transition hover:text-teal-400 px-1.5 py-2 nav-link ${activeLink === "records" ? "text-teal-400" : ""
                    }`}
                  onClick={(e) => handleLinkClick(e, "records")}
                >
                  Records
                  <span
                    className={`absolute inset-x-0 -bottom-[0.2rem] h-px bg-gradient-to-r from-teal-400/0 via-teal-400/40 to-teal-400/0 transition-opacity duration-300 ease-in-out ${activeLink === "records" ? "opacity-100" : "opacity-0"
                      }`}
                  ></span>
                </Link>
                {showBlog && (
                  <>
                    <Link
                      to="/blog"
                      className={`relative block transition hover:text-teal-400 px-1.5 py-2 nav-link ${activeLink === "blog" ? "text-teal-400" : ""
                        }`}
                      onClick={(e) => handleLinkClick(e, "blog")}
                    >
                      Blog
                      <span
                        className={`absolute inset-x-0 -bottom-[0.2rem] h-px bg-gradient-to-r from-teal-400/0 via-teal-400/40 to-teal-400/0 transition-opacity duration-300 ease-in-out ${activeLink === "blog" ? "opacity-100" : "opacity-0"
                          }`}
                      ></span>
                    </Link>
                  </>
              ...
              <div className="md:hidden py-2 flex items-center gap-3">
                ...
                <button
                  ...
                  className={`p-1.5 rounded-full transition-all duration-200 ${isMusicPlaying
                ...
                {/* Performance Mode Toggle - Mobile (only show at sm+ where animation is visible) */}
                <button
                  onClick={toggleMode}
                  className={`hidden sm:block md:hidden p-1.5 rounded-full transition-all duration-200 ${mode === 'high'
                    ? 'text-teal-400 hover:text-teal-300'
                    : 'text-zinc-400 hover:text-zinc-200'
                    }`}
                  style={{
                  ...
                  title={mode === 'high' ? 'Switch to 2D mode' : 'Switch to 3D mode'}
                >
                  {mode === 'high' ? (
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
        ...
        <div className="hidden md:flex items-center gap-2" style={{
          ...
          <button
            onClick={toggleMode}
            className={`p-2 rounded-full transition-all duration-200 ${mode === 'high'
              ? 'text-teal-400 hover:text-teal-300'
              : 'text-zinc-400 hover:text-zinc-200'
              }`}
            style={{
            ...
            title={mode === 'high' ? 'Switch to low-performance mode (2D)' : 'Switch to high-performance mode (3D)'}
          >
            {mode === 'high' ? (
          ...
          <button
            onClick={handleMusicToggle}
            className={`p-2 rounded-full transition-all duration-200 ${isMusicPlaying
              ? 'text-teal-400 hover:text-teal-300'
              : 'text-zinc-400 hover:text-zinc-200'
              }`}
            style={{
      ...
      {isOpen && (
        <div
          ref={menuRef}
          className={`mx-auto px-4 py-8 rounded-3xl mt-8 mx-3 transition-all duration-300 ${isAnimating ? "opacity-100 scale-100" : "opacity-0 scale-50"
            }`}
          style={{
            position: 'fixed',
            top: '5rem',
      ...
      {isOpen && (
        <div
          className={`fixed inset-0 transition-all duration-300 ${isAnimating ? "opacity-100" : "opacity-0"
            }`}
          style={{
            top: 0,
            left: 0,
